...
bool BGTactics::moveToStart(bool force)
{
    ...
    if (bgType == BATTLEGROUND_WS)
    {
        ...
        if (startSpot == BB_WSG_WAIT_SPOT_RIGHT)
        {
            if (bot->GetTeamId() == TEAM_HORDE)
                MoveTo(bg->GetMapId(), WS_WAITING_POS_HORDE_1.GetPositionX() + frand(-2.0f, 2.0f), WS_WAITING_POS_HORDE_1.GetPositionY() + frand(-2.0f, 2.0f), WS_WAITING_POS_HORDE_1.GetPositionZ());
            else
                MoveTo(bg->GetMapId(), WS_WAITING_POS_ALLIANCE_1.GetPositionX() + frand(-2.0f, 2.0f), WS_WAITING_POS_ALLIANCE_1.GetPositionY() + frand(-2.0f, 2.0f), WS_WAITING_POS_ALLIANCE_1.GetPositionZ());
        }
        else if (startSpot == BB_WSG_WAIT_SPOT_LEFT)
        {
            if (bot->GetTeamId() == TEAM_HORDE)
                MoveTo(bg->GetMapId(), WS_WAITING_POS_HORDE_2.GetPositionX() + frand(-2.0f, 2.0f), WS_WAITING_POS_HORDE_2.GetPositionY() + frand(-2.0f, 2.0f), WS_WAITING_POS_HORDE_2.GetPositionZ());
            else
    ...
    {
        if (bot->GetTeamId() == TEAM_HORDE)
            MoveTo(bg->GetMapId(), EY_WAITING_POS_HORDE.GetPositionX() + frand(-3.0f, 3.0f), EY_WAITING_POS_HORDE.GetPositionY() + frand(-3.0f, 3.0f), EY_WAITING_POS_HORDE.GetPositionZ());
        else
            MoveTo(bg->GetMapId(), EY_WAITING_POS_ALLIANCE.GetPositionX() + frand(-3.0f, 3.0f), EY_WAITING_POS_ALLIANCE.GetPositionZ() + frand(-3.0f, 3.0f), EY_WAITING_POS_ALLIANCE.GetPositionZ());
    }
    else if (bgType == BATTLEGROUND_IC)
    {
        ...
        if (bot->GetTeamId() == TEAM_HORDE)
        {
            ...
            {
                if (urand(0, 1))
                    MoveTo(bg->GetMapId(), IC_WEST_WAITING_POS_HORDE.GetPositionX() + frand(-5.0f, 5.0f), IC_WEST_WAITING_POS_HORDE.GetPositionY() + frand(-5.0f, 5.0f), IC_WEST_WAITING_POS_HORDE.GetPositionZ());
                else
                    MoveTo(bg->GetMapId(), IC_WAITING_POS_HORDE.GetPositionX() + frand(-5.0f, 5.0f), IC_WAITING_POS_HORDE.GetPositionY() + frand(-5.0f, 5.0f), IC_WAITING_POS_HORDE.GetPositionZ());
            }
            else
            {
                if (urand(0, 1))
                    MoveTo(bg->GetMapId(), IC_EAST_WAITING_POS_HORDE.GetPositionX() + frand(-5.0f, 5.0f), IC_EAST_WAITING_POS_HORDE.GetPositionY() + frand(-5.0f, 5.0f), IC_EAST_WAITING_POS_HORDE.GetPositionZ());
                else
                    MoveTo(bg->GetMapId(), IC_WAITING_POS_HORDE.GetPositionX() + frand(-5.0f, 5.0f), IC_WAITING_POS_HORDE.GetPositionY() + frand(-5.0f, 5.0f), IC_WAITING_POS_HORDE.GetPositionZ());
            }
        }
        else
        {
            if (role < 3)
            {
                if (urand(0, 1))
                    ...
                    MoveTo(bg->GetMapId(), IC_WAITING_POS_ALLIANCE.GetPositionX() + frand(-5.0f, 5.0f), IC_WAITING_POS_ALLIANCE.GetPositionY() + frand(-5.0f, 5.0f), IC_WAITING_POS_ALLIANCE.GetPositionZ());
            }
            else
            {
                if (urand(0, 1))
    ...
    switch (bgType)
    {
            ...
            if (bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) || bot->HasAura(BG_WS_SPELL_SILVERWING_FLAG))
            {
                if (bot->GetTeamId() == TEAM_ALLIANCE)
                {
                    if (teamFlagTaken())
                    {
                        ...
                        pos.Set(hidePos.GetPositionX(), hidePos.GetPositionY(), hidePos.GetPositionZ(), bot->GetMapId());
                    }
                    else
                ...
                else
                {
                    if (teamFlagTaken())
                    {
                        ...
                        pos.Set(WS_FLAG_POS_HORDE.GetPositionX(), WS_FLAG_POS_HORDE.GetPositionY(), WS_FLAG_POS_HORDE.GetPositionZ(), bot->GetMapId());
                }
        ...
        case BATTLEGROUND_IC:
        {
            ...
            if (bot->GetTeamId() == TEAM_HORDE) // HORDE
            {
                ...
                {
                    if (Creature* allyboss = bg->GetBGCreature(BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE))
                    {
                        if (allyboss->IsVisible())
                        {
                ...
                if (!BgObjective)
                {
                    ...
                    if (role > 10) // disabled
                    {
                        ...
                        if (firstTower < 2)
                        {
                            pos.Set(IC_CANNON_POS_HORDE1.GetPositionX(), IC_CANNON_POS_HORDE1.GetPositionY(), IC_CANNON_POS_HORDE1.GetPositionZ(), bg->GetMapId());
                            posMap["bg objective"] = pos;
                            return true;
                        }
                        if (secondTower < 2)
                        {
                            pos.Set(IC_CANNON_POS_HORDE2.GetPositionX(), IC_CANNON_POS_HORDE2.GetPositionY(), IC_CANNON_POS_HORDE2.GetPositionZ(), bg->GetMapId());
                            posMap["bg objective"] = pos;
                            return true;
                        }
                    }
                            ...
                            else
                            {
                                if (GameObject* gate = bg->GetBGObject(BG_IC_GO_ALLIANCE_GATE_3))
                                {
                                    if (controlsVehicle)
                                    {
                                        ...
                                        if (vehicleId == NPC_SIEGE_ENGINE_H)
                                        {
                                            BgObjective = gate;
                                        }
                                        else
                    ...
                    if (!BgObjective && gateOpen) // Capture Keep
                    {
                        ...
                        ICNodePoint const& nodePoint = isleOfConquestBG->GetICNodePoint(NODE_TYPE_GRAVEYARD_A);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_H && nodePoint.nodeState != NODE_STATE_CONTROLLED_H)
                        {
            ...
            if (bot->GetTeamId() == TEAM_ALLIANCE) // ALLIANCE
            {
                ...
                if (allCaptured) // target enemy boss
                {
                    if (Creature* hordeboss = bg->GetBGCreature(BG_IC_NPC_OVERLORD_AGMAR))
                    {
                        if (!hordeboss->IsVisible())
                        {
                ...
                if (!BgObjective)
                {
                    ...
                    if (role > 10) // disabled
                    {
                        ...
                        if (firstTower < 3)
                        {
                            pos.Set(IC_CANNON_POS_ALLIANCE1.GetPositionX(), IC_CANNON_POS_ALLIANCE1.GetPositionY(), IC_CANNON_POS_ALLIANCE1.GetPositionZ(), bg->GetMapId());
                            posMap["bg objective"] = pos;
                            return true;
                        }
                        if (secondTower < 3)
                        {
                            pos.Set(IC_CANNON_POS_ALLIANCE2.GetPositionX(), IC_CANNON_POS_ALLIANCE2.GetPositionY(), IC_CANNON_POS_ALLIANCE2.GetPositionZ(), bg->GetMapId());
                            posMap["bg objective"] = pos;
                            return true;
                        }
                    }
                    ...
                    if (!BgObjective || controlsVehicle) // Check gates
                    {
                        ...
                        if (GameObject* pGO = bg->GetBGObject(BG_IC_GO_HORDE_KEEP_PORTCULLIS))
                        {
                            ...
                            {
                                if (GameObject* gate = bg->GetBGObject(BG_IC_GO_HORDE_GATE_3))
                                {
                                    if (controlsVehicle)
                                    {
                                        // come close to gate if siege engine
                                        if (vehicleId == NPC_SIEGE_ENGINE_A)
                                        {
                                            BgObjective = gate;
                                        }
                                        else
                    ...
                    if (!BgObjective && gateOpen) // Capture Keep
                    {
                        ...
                        ICNodePoint const& nodePoint = isleOfConquestBG->GetICNodePoint(NODE_TYPE_GRAVEYARD_H);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_A && nodePoint.nodeState != NODE_STATE_CONTROLLED_A)
                        {
...
bool BGTactics::useBuff()
{
    ...
    bool needRegen = bot->GetHealthPct() < sPlayerbotAIConfig->mediumHealth || (AI_VALUE2(bool, "has mana", "self target") && AI_VALUE2(uint8, "mana", "self target") < sPlayerbotAIConfig->mediumMana);
    bool needSpeed = (bgType != BATTLEGROUND_WS || bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) || bot->HasAura(BG_WS_SPELL_SILVERWING_FLAG) || bot->HasAura(BG_EY_NETHERSTORM_FLAG_SPELL)) || !(teamFlagTaken() || flagTaken());
...
bool BGTactics::IsLockedInsideKeep()
{
    ...
    bool isInside = false;
    if (bot->GetTeamId() == TEAM_ALLIANCE && bot->GetPositionX() < 410.0f && bot->GetPositionY() > -900.0f && bot->GetPositionY() < -765.0f)
        isInside = true;
    if (bot->GetTeamId() == TEAM_HORDE && bot->GetPositionX() > 1153.0f && bot->GetPositionY() > -849.0f && bot->GetPositionY() < -679.0f)
        isInside = true;
